...
//! （standalone transfer agentの設定キー）。カスタム転送名は`rclone-storage`とし
//! （Issue #2 3節「custom transfer名は`rclone-storage`とする」）、
...
const カスタム転送名: &str = "rclone-storage";

pub(crate) const 設定キー一覧: [&str; 4] = [
    "lfs.customtransfer.rclone-storage.path",
    "lfs.customtransfer.rclone-storage.concurrent",
    "lfs.customtransfer.rclone-storage.direction",

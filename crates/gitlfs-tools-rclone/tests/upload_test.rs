...
use gitlfs_tools_domain::{オブジェクト識別子, 期待バイト数, 検証前のローカルファイル, 検証済みローカルファイル};
use gitlfs_tools_storage_port::{アップロード結果, オブジェクト保管庫};
...
fn 既に違うサイズで存在すれば整合性エラーで転送しない() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(
        結果,
        Err(gitlfs_tools_domain::保管エラー::整合性(gitlfs_tools_domain::整合性エラー::バイト数の不一致 { .. }))
    ));

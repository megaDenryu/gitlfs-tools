...
///
/// 注意: `プロファイル名`は`domain`層の値型でありHash実装を持たないため、内部保持は
/// `HashMap`でなく`Vec`にしている。件数はPC1台あたりのプロファイル数程度であり、
/// `プロファイルを解決する`の線形探索で性能上の問題はない（生値の`String`キーへ
/// 剥がすより、検証済みドメイン型のまま保持することを優先した）。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PC設定 {
    スキーマ版: 設定スキーマ版,
    プロファイル一覧: Vec<(プロファイル名, PCプロファイル)>,
}

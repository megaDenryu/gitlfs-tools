...
use gitlfs_tools_domain::{Rclone実行ファイルの場所, プロファイル名};
...
fn rclone子プロセス方式の設定値を取り出す(
    プロファイル: &PCプロファイル,
) -> Result<(&gitlfs_tools_domain::Rcloneリモート名, &Rclone実行ファイルの場所), Box<dyn std::error::Error>> {
    match プロファイル.保管先() {
...
fn 正常なpc設定を解析しプロファイルを解決する() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(一時アップロード先.文字列表現().starts_with("gitlfs-tools/lfs/tmp/"));
    Ok(())
}

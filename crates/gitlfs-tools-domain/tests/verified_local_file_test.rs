//! `検証済みローカルファイル`の生成条件（バイト数とSHA-256の両方一致）のテスト。

use std::io::Write;

use gitlfs_tools_domain::{オブジェクト識別子, 保管エラー, 整合性エラー, 検証前のローカルファイル, 期待バイト数};
...
#[test]
fn バイト数とハッシュが両方一致すれば検証済みローカルファイルを生成できる() -> Result<(), Box<dyn std::error::Error>> {
    let 内容 = b"gitlfs-tools issue3 fixture";

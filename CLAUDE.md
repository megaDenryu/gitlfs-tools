...
実マウント先が要るのは、Google Drive for Desktop 自身の挙動（クラウドへの同期の遅れ、ストリーミング状態のファイルの読み出し、ドライブ文字の割り当て）を調べるときだけである。基盤の実装の検証にマウントは要らない（`lfs-rclone-local` は標準ライブラリのファイル操作しか使わない）。担当者がその調査を行う場合も、保管先の根ではなく調査専用のフォルダを作り、その中だけで行う。

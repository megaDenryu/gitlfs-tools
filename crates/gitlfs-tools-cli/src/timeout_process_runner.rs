...
//! `gitlfs-tools-rclone`crateの`Rcloneプロセス実行器`と同じ責務を持つが、そちらは
//! `pub(crate)`でクレートをまたいで参照できないため、doctorの書き込み確認専用として
//! ここへ複製する（このタスクで触ってよい範囲が`gitlfs-tools-cli`配下に限られるため）。
...
use gitlfs_tools_domain::{保管エラー, Rclone実行ファイルの場所, 転送タイムアウト};

...
use gitlfs_tools_domain::保管先基底パス;
...
/// PC設定の1プロファイルを解決した値。`gitlfs-tools-rclone`・`gitlfs-tools-local`（保管庫の実装）
/// と`gitlfs-tools-protocol`がそのまま受け取れる、名前のある型である。この層は保管先へ触らない
/// （rcloneを起動せず、ファイルシステムの状態も見ない）。
///
/// ローカルの一時ファイルの置き場所は持たない。ダウンロードの一時ファイルはGit LFSが
/// renameで奪うため、置き場所はPC設定ではなくリポジトリ側から決まる
/// （`gitlfs-tools-cli`の`git_lfs_storage_directory.rs`）。

...
fn installはローカル設定だけを登録しグローバル設定を変更しない() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let パス値 = ローカル設定を読む(リポジトリ.path(), "lfs.customtransfer.gitlfs-tools.path")?;
    assert!(!パス値.trim().is_empty(), "実行ファイルパスが登録されているべき");
    assert_eq!(ローカル設定を読む(リポジトリ.path(), "lfs.customtransfer.gitlfs-tools.concurrent")?.trim(), "true");
    assert_eq!(ローカル設定を読む(リポジトリ.path(), "lfs.customtransfer.gitlfs-tools.direction")?.trim(), "both");
    assert_eq!(ローカル設定を読む(リポジトリ.path(), "lfs.standalonetransferagent")?.trim(), "gitlfs-tools");
...
fn installを再実行しても同じ値へ収束する() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        ローカル設定を読む(リポジトリ.path(), "lfs.customtransfer.gitlfs-tools.direction")?,
        "both\n"
    );
    Ok(())
}

...
pub(crate) fn 複製を実行する(複製元: 複製元リポジトリURL, 複製先の指定: Option<複製先ディレクトリ>) -> ExitCode {
    ...
    let 操作 = 対象リポジトリのGitLFS操作::生成する(複製先.作業ツリールートへ変換する());
...
fn 複製した作業ツリーを使える状態にする(操作: &対象リポジトリのGitLFS操作) -> Result<(), 複製エラー> {
    ...
    let 実行場所 = Gitコマンドの実行場所::作業ツリールートから生成する(操作.作業ツリーのルート());
    install_command::対象リポジトリへ設定を登録する(実行場所, None)
        .map_err(|エラー| 複製エラー::転送agentの登録に失敗 { 説明: エラー.to_string() })?;
...
fn 論理プロファイルが解決できることを確かめる(操作: &対象リポジトリのGitLFS操作) -> Result<(), 複製エラー> {
    ...
    let 置き場所 = プロファイル解決に使う設定の置き場所::生成する(操作.作業ツリーのルート().プロジェクト設定の探索起点(), 場所);
    置き場所
        .論理プロファイルを解決する()
        .map(|_| ())
...
fn 続けて示す案内を表示する(エラー: &複製エラー) {
    for 行 in エラー.続けて示す案内() {
        eprintln!("{行}");
    }
}

...
//!
//! テスト専用の環境変数`GITLFS_TOOLS_PC_CONFIG_DIR`が設定されていれば、実ユーザーの設定
//! ディレクトリの代わりにそのディレクトリを使う（結合テストが実設定ディレクトリを
//! 読み書きしないための差し替え口。`gitlfs-tools-rclone`の`GITLFS_TOOLS_TEST_EXECUTABLE`と
//! 同じ方針）。
...
const PC設定ディレクトリ環境変数: &str = "GITLFS_TOOLS_PC_CONFIG_DIR";
